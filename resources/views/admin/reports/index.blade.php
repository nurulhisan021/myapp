...
@section('admin_content')
    <h1 class="text-2xl font-bold mb-6">รายงานและภาพรวม</h1>
        ...
        <div class="p-4 border-b">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-4 text-sm">
                    <a href="{{ route('admin.reports.index', ['period' => 'all']) }}" class="{{ $period == 'all' ? 'font-bold text-brand' : 'text-gray-500 hover:text-brand' }}">ทั้งหมด</a>
                    <a href="{{ route('admin.reports.index', ['period' => 'today']) }}" class="{{ $period == 'today' ? 'font-bold text-brand' : 'text-gray-500 hover:text-brand' }}">วันนี้</a>
                    <a href="{{ route('admin.reports.index', ['period' => 'week']) }}" class="{{ $period == 'week' ? 'font-bold text-brand' : 'text-gray-500 hover:text-brand' }}">สัปดาห์นี้</a>
                    <a href="{{ route('admin.reports.index', ['period' => 'month']) }}" class="{{ $period == 'month' ? 'font-bold text-brand' : 'text-gray-500 hover:text-brand' }}">เดือนนี้</a>
                    <a href="{{ route('admin.reports.index', ['period' => 'year']) }}" class="{{ $period == 'year' ? 'font-bold text-brand' : 'text-gray-500 hover:text-brand' }}">ปีนี้</a>
                </div>
            </div>
        </div>
            ...
            <tbody>
                @forelse ($recentOrders as $order)
                    <tr class="border-b hover:bg-gray-50">
                        <td class="p-4">{{ $recentOrders->firstItem() + $loop->index }}</td>
